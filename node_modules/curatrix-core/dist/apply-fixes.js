...
async function buildPlanChanges(rootDir, issue, patchPreview) {
...
function buildPatchChange(issue) {
    if (!issue.patch) {
        return undefined;
    }
    return {
...
function extractCurrentDependencyFromIssue(issue) {
    const evidence = issue.evidence.find((entry) => entry.label === "version" || entry.label === "package");
    return evidence?.value;
}
function inferChangeType(issue, patchLikeText) {
...
function extractPatchedContent(patchText) {
...
function applySimpleUnifiedPatch(original, patchText) {
...
//# sourceMappingURL=apply-fixes.js.map
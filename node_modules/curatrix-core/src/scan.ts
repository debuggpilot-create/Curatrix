...
import { createIssueId, emptySummary, type BaselineDeltaSummary, type BaselineSnapshot, type Category, type Evidence, type FixOptions, type FixPlan, type Issue, type Location, type ScanOptions, type ScanResult, type Severity } from "./types.js";
import { loadCuratrixConfig, type CuratrixConfig } from "./config.js";
...
interface CandidateIssue {
  ...
  evidence: Evidence[];
  locations: Location[];
  fixAvailability: "none" | "preview" | "apply";
  source: "static" | "ai";
  remediation?: string;
  patch?: string;
  depth?: number;
  reputation?: Issue["reputation"];
  vexStatus?: Issue["vexStatus"];
}
...
async function scanDependencies(
  ...
  files: string[],
  options: ScanOptions,
  config: CuratrixConfig,
  vexEntries: VexEntry[],
): Promise<CandidateIssue[]> {
  const packageJsonPath = path.join(rootDir, "package.json");
  const packageJson = await readJsonFile<PackageJsonFile>(packageJsonPath);
  if (!packageJson) {
    return [];
  }
...
async function discoverDependencyUsage(files: string[], packageNames: string[]): Promise<Set<string>> {
...
async function resolveNodeDependencies(
  ...
  const lockfile = await readJsonFile<PackageLockFile>(path.join(rootDir, "package-lock.json"));
...
function collectFromPackageLockPackages(
  packages: Record<string, { version?: string; dependencies?: Record<string, string> }>,
  resolved: Map<string, ResolvedDependency>,
  maxDepth: number,
): void {
  const queue: Array<{ packagePath: string; depth: number }> = [{ packagePath: "", depth: 0 }];
...
function collectFromPackageLockDependencies(
  dependencies: Record<string, PackageLockDependency>,
  resolved: Map<string, ResolvedDependency>,
  depth: number,
  maxDepth: number,
...
function addResolvedDependency(resolved: Map<string, ResolvedDependency>, name: string, version: string, depth: number): void {
...
function compareResolvedDependencies(a: ResolvedDependency, b: ResolvedDependency): number {
...
function containsSuspiciousInstallScript(scriptValue: string): boolean {
...
async function collectDependencyReputation(rootDir: string, dependencies: ResolvedDependency[]): Promise<Map<string, ReputationRecord>> {
  const reputation = new Map<string, ReputationRecord>();
...
async function getPackageReputation(rootDir: string, pkgName: string, pkgVersion: string): Promise<ReputationRecord | undefined> {
  ...
  try {
    ...
    const payload = await response.json() as {
      ...
      versions?: Record<string, { author?: { name?: string } | string; maintainers?: Array<{ name?: string }> }>;
...
async function fetchAuthorPackageCount(authorName: string): Promise<number | undefined> {
  try {
...
function isSuspiciousVersionJump(pkgVersion: string, timeMap: Record<string, string>): boolean {
  ...
  let latestPreOnePublishedAt: number | undefined;
...
async function readReputationCache(rootDir: string): Promise<Record<string, { cachedAt: string; record: ReputationRecord }>> {
  const target = path.join(rootDir, ".curatrix", "cache", "reputation.json");
  try {
    return (await readJsonFile<Record<string, { cachedAt: string; record: ReputationRecord }>>(target)) ?? {};
  } catch {
...
async function writeReputationCache(rootDir: string, cache: Record<string, { cachedAt: string; record: ReputationRecord }>): Promise<void> {
  const target = path.join(rootDir, ".curatrix", "cache", "reputation.json");
  try {
...
async function loadVexEntries(rootDir: string, config: CuratrixConfig): Promise<VexEntry[]> {
  ...
  try {
    const payload = await readJsonFile<{
      product_tree?: { full_product_names?: Array<{ name?: string; product_id?: string }> };
      vulnerabilities?: Array<{
        cve?: string;
        cve_id?: string;
        product_status?: Partial<Record<"not_affected" | "fixed", string[]>>;
      }>;
    }>(configuredPath);
    ...
    for (const vulnerability of payload.vulnerabilities) {
      ...
      for (const status of ["not_affected", "fixed"] as const) {
...
function resolveVexStatus(
  vexEntries: VexEntry[],
  packageName: string,
  packageVersion: string | undefined,
  aliases: string[],
): "not_affected" | "fixed" | undefined {
  ...
  for (const alias of aliases) {
    ...
    const match = vexEntries.find((entry) =>
      entry.cve.toUpperCase() === normalizedAlias
...
function advisoryAliases(advisory: string): string[] {
  return advisory.split("|").map((part) => part.trim()).filter((part) => /^CVE-/i.test(part));
}

async function scanSecrets(rootDir: string, files: string[]): Promise<CandidateIssue[]> {
  const issues: CandidateIssue[] = [];
  const patterns = [
    { ruleId: "secrets.aws-key", regex: /AKIA[0-9A-Z]{16}/g, title: "Possible AWS access key detected", severity: "critical" as Severity },
    { ruleId: "secrets.stripe-key", regex: /sk_(?:live|test)_[0-9A-Za-z]{16,}/g, title: "Possible Stripe secret key detected", severity: "critical" as Severity },
    { ruleId: "secrets.jwt", regex: /eyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9._-]{10,}\.[A-Za-z0-9._-]{10,}/g, title: "Possible JWT token detected", severity: "high" as Severity },
    { ruleId: "secrets.private-key", regex: /-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/g, title: "Private key material detected", severity: "critical" as Severity },
  ];
...
async function scanGitHistory(rootDir: string): Promise<CandidateIssue[]> {
  ...
  for (const expression of expressions) {
    try {
      ...
      let currentMeta: { hash: string; author: string; date: string } | undefined;
...
async function scanInfrastructure(rootDir: string, files: string[]): Promise<CandidateIssue[]> {
  const issues: CandidateIssue[] = [];
...
async function scanAiAgent(rootDir: string, files: string[]): Promise<CandidateIssue[]> {
  const issues: CandidateIssue[] = [];
...
export async function saveBaseline(rootDir: string, result: ScanResult): Promise<string> {
  const config = await loadCuratrixConfig(rootDir);
...
export async function compareWithBaseline(rootDir: string, result: ScanResult): Promise<ScanResult> {
  const config = await loadCuratrixConfig(rootDir);
  const snapshot = await readJsonFile<BaselineSnapshot>(baselinePath(rootDir, config.baselineDir));
  if (!snapshot) {
    return {
      ...result,
  ...
  const baselineDelta: BaselineDeltaSummary = { new: 0, resolved: 0, unchanged: 0 };
  const issues = result.issues.map((issue) => {
    const baselineStatus: "unchanged" | "new" = baselineSet.has(issue.fingerprint) ? "unchanged" : "new";
...
export async function createFixPlan(rootDir: string, issueId: string, options?: { vulnerabilityProvider?: ScanOptions["vulnerabilityProvider"] }): Promise<FixPlan> {
...
export async function applyFix(options: FixOptions & { issueId: string }): Promise<FixPlan> {
...
function baselinePath(rootDir: string, baseDir: string): string {
...
function applyConfigToIssues(issues: CandidateIssue[], config: CuratrixConfig): CandidateIssue[] {
  return dedupeIssues(
    issues
...
function applyConfigToAiIssues(issues: Issue[], config: CuratrixConfig): Issue[] {
  return dedupeFinalIssues(
    issues
      .filter((issue) => !config.ignoredRuleIds.includes(issue.ruleId))
      .map((issue) => ({
        ...issue,
...
function fixSummary(ruleId: string): string {
  switch (ruleId) {
...
function renderPatchedContent(ruleId: string, original: string): string {
  switch (ruleId) {
    case "secrets.env-not-ignored":
...
function dedupeIssues(issues: CandidateIssue[]): CandidateIssue[] {
...
function dedupeFinalIssues(issues: Issue[]): Issue[] {
  const seen = new Set<string>();
...
function finalizeIssue(candidate: CandidateIssue): Issue {
...
function createSimplePatch(file: string, before: string, after: string): string {
